...
public class TaskConfiguration extends RunConfigurationBase {
  ...
  public SettingsEditor<? extends RunConfiguration> getConfigurationEditor() {
    return new SettingsEditor<TaskConfiguration>() {
      private final TaskSettingsComponent component =
          new TaskSettingsComponent(getProject(), false);

      @Override
      protected void resetEditorFrom(@NotNull TaskConfiguration settings) {
        component.setTaskData(new TaskData(getName(), className, cppPath, input,
                                           output, testType, new Test[0]));
      }
  ...
  @Override
  public @Nullable
  CidrCommandLineState getState(@NotNull Executor executor,
                                @NotNull ExecutionEnvironment environment) {
    //        RunConfiguration configuration =
    //        TaskRunner.getRunnerSettings(getProject()).getConfiguration();
    //		return new CidrCommandLineState(environment, new
    //CMakeLauncher(environment, (CMakeAppRunConfiguration)configuration));
  ...
  private static StreamConfiguration
  readStreamConfiguration(Element element, String typeAttribute,
                          String filenameAttribute) {
    try {
      StreamConfiguration.StreamType inputType =
          StreamConfiguration.StreamType.valueOf(
              element.getAttribute(typeAttribute).getValue());
      if (inputType.hasStringParameter) {
        return new StreamConfiguration(
            inputType, element.getAttributeValue(filenameAttribute));
      } else {
  ...
  public Test[] getTests() { return Arrays.copyOf(tests, tests.length); }

  public String getCppPath() { return cppPath; }

  public String getClassName() { return className; }

  public TestType getTestType() { return testType; }

  public StreamConfiguration getInput() { return input; }

  public StreamConfiguration getOutput() { return output; }
}
