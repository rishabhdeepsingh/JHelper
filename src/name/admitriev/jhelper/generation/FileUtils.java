...
public class FileUtils {
  ...
  public static OutputWriter getOutputWriter(VirtualFile virtualFile,
                                             Object requester) {
    try {
  ...
  @Nullable
  private static VirtualFile findChild(VirtualFile file,
  ...
  public static VirtualFile findOrCreateByRelativePath(VirtualFile root,
                                                       String localPath) {
    return ApplicationManager.getApplication().runWriteAction(
        new Computable<>() {
          ...
          public VirtualFile compute() {
            ...
            if (index < path.length()) {
              return findOrCreateByRelativePath(child,
  ...
  public static String relativePath(String parentPath, String childPath) {
    ...
    if (!isChild(parentPath, childPath)) {
      throw new IllegalArgumentException(
          "childPath should be inside a parentPath");
    }
    // Minimum is needed for case when childPath = parentPath and there's no /
    // at the end of childPath
    return childPath.substring(
        Math.min(parentPath.length(), childPath.length()));
  }
