...
public class DeletionMarkingVisitor extends OCVisitor {
  ...
  public DeletionMarkingVisitor(Collection<PsiElement> toDelete,
  ...
  private static boolean isParentFor(OCElement potentialParent,
                                     PsiElement potentialChild) {
  ...
  private void removeIfNoReference(OCElement element) {
    for (PsiReference reference :
         ReferencesSearch.search(element, searchScope)) {
