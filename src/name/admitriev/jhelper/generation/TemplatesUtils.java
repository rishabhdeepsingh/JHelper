...
public class TemplatesUtils {
  ...
  public static final Pattern TASK_FILE =
      Pattern.compile("%TaskFile%", Pattern.LITERAL);
  public static final Pattern TESTS =
      Pattern.compile("%Tests%", Pattern.LITERAL);
  public static final Pattern SOLVER_CALL =
      Pattern.compile("%SolverCall%", Pattern.LITERAL);
  public static final Pattern INPUT =
      Pattern.compile("%Input%", Pattern.LITERAL);
  public static final Pattern OUTPUT =
      Pattern.compile("%Output%", Pattern.LITERAL);
  public static final Pattern CODE = Pattern.compile("%Code%", Pattern.LITERAL);

  private TemplatesUtils() {}

  public static String replaceAll(String text, Pattern pattern,
                                  String replacement) {
  ...
  public static String getTemplate(Project project, String name) {
    ...
    if (file == null) {
      ...
      if (file == null) {
        throw new JHelperException("Can't open template file(" + filename +
                                   ") after its creation");
      }
    }
  ...
  private static void createTemplateFromDefault(Project project, String name) {
    String filename = name + ".template";
    VirtualFile file =
        FileUtils.findOrCreateByRelativePath(project.getBaseDir(), filename);
    PsiFile psiFile = PsiManager.getInstance(project).findFile(file);
    String defaultTemplate;
    try {
      defaultTemplate =
          getResourceContent("/name/admitriev/jhelper/templates/" + filename);
    } catch (IOException e) {
      throw new NotificationException(
          "Couldn't open default template " + filename, e);
    }
