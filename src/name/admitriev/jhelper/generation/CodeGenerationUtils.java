...
public class CodeGenerationUtils {
   ...
   */
  public static void generateRunFile(@NotNull Project project,
  ...
  private static void generateSubmissionFile(@NotNull Project project,
                                             @NotNull PsiFile inputFile,
                                             @NotNull TaskConfiguration task) {
    if (FileUtils.isNotCppFile(inputFile)) {
      throw new NotificationException("Not a cpp file",
    ...
    FileUtils.writeToFile(psiOutputFile,
  ...
  generateRunFileContent(Project project, TaskConfiguration task, String path) {
    String template = TemplatesUtils.getTemplate(project, "run");
    template =
        TemplatesUtils.replaceAll(template, TemplatesUtils.TASK_FILE, path);
    template =
        TemplatesUtils.replaceAll(template, TemplatesUtils.TESTS,
                                  generateTestDeclaration(task.getTests()));
    template = TemplatesUtils.replaceAll(template, TemplatesUtils.CLASS_NAME,
                                         task.getClassName());
    template =
        TemplatesUtils.replaceAll(template, TemplatesUtils.SOLVER_CALL,
  ...
  private static String generateSubmissionFileContent(Project project,
                                                      String code,
    ...
    template = TemplatesUtils.replaceAll(template, TemplatesUtils.CLASS_NAME,
                                         task.getClassName());
    template = TemplatesUtils.replaceAll(template, TemplatesUtils.INPUT,
                                         getInputDeclaration(task));
    template = TemplatesUtils.replaceAll(template, TemplatesUtils.OUTPUT,
                                         getOutputDeclaration(task));
    template =
        TemplatesUtils.replaceAll(template, TemplatesUtils.SOLVER_CALL,
                                  generateSolverCall(task.getTestType()));
    return template;
  ...
  private static String getOutputDeclaration(TaskConfiguration task) {
    String outputFileName =
        task.getOutput().getFileName(task.getName(), ".out");
    if (outputFileName == null) {
      return "";
    } else if (task.getOutput().type ==
               StreamConfiguration.StreamType.LOCAL_REGEXP) {
      return "freopen(\"" + outputFileName + "\", \"w\", stdout);";
    }
    throw new NotificationException("Your task is in inconsistent state",
                                    "Can't output to local regexp");
  }

  private static String getInputDeclaration(TaskConfiguration task) {
    if (task.getInput().type == StreamConfiguration.StreamType.LOCAL_REGEXP) {
      return "freopen(getLastFileName(" + quote(task.getInput().fileName) +
          ").c_str(), \"r\", stdin);";
    } else if (task.getInput().type ==
               StreamConfiguration.StreamType.STANDARD) {
      return "";
    } else {
      return "freopen(\"" + task.getInput().getFileName(task.getName(), ".in") +
          "\", \"r\", stdin);";
    }
  }
  ...
  private static @NotNull PsiFile getOutputFile(Project project) {
    ...
    VirtualFile outputFile = project.getBaseDir().findFileByRelativePath(
    ...
    PsiFile psiOutputFile =
        PsiManager.getInstance(project).findFile(outputFile);
  ...
  private static PsiFile getRunFile(Project project) {
    ...
    VirtualFile outputFile =
        project.getBaseDir().findFileByRelativePath(configuration.getRunFile());
    if (outputFile == null) {
      throw new NotificationException(
          "No run file found.",
  ...
  private static void removeUnusedCode(PsiFile file) {
    while (true) {
      ...
      SearchScope scope =
          GlobalSearchScope.fileScope(project, file.getVirtualFile());
   ...
   * Adds main function, inlines all used code except standard library and puts
   * it to output file from configuration
   *
   * @param project Project to get configuration from
   */
  public static void
  generateSubmissionFileForTask(@NotNull Project project,
                                @NotNull TaskConfiguration taskConfiguration) {
    String pathToClassFile = taskConfiguration.getCppPath();
    VirtualFile virtualFile =
        project.getBaseDir().findFileByRelativePath(pathToClassFile);
    if (virtualFile == null) {
      throw new NotificationException(
          "Task file not found", "Seems your task is in inconsistent state");
    }
